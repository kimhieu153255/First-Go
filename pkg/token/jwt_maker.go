...
func (jwtMaker *JWTMaker) CreateToken(userID int64, email, role, fullname string, duration time.Duration) (string, *Payload, error) {
	payload, err := NewPayload(userID, email, fullname, role, duration)
	if err != nil {
		return "", payload, err
	}
